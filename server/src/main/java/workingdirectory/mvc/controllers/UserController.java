...
public class UserController {
    ...
    @GetMapping("/{account}")
    public User getUserInformation(@PathVariable String account){
        User user = userService.findById(account).orElseThrow();
        return user;
    }

    @PutMapping("/{account}")
    public ResponseEntity<User> updateUser(@PathVariable String account, @RequestBody User user) throws JsonProcessingException {
